...
class GolfDB(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        normalize_coor_pose = normalize_screen_coordinates(poses[:,:,:2], w, h).reshape(-1,2,17)
        norm_coor_pose_conf = np.concatenate((normalize_coor_pose, poses[:,:,-1:].reshape(-1,1,17)),axis=1).reshape(-1,51)
        sample = {'inputs':norm_coor_pose_conf, 'labels':np.asarray(labels)}
        if self.transform:
