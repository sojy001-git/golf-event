...
class EventDetector(nn.Module):
    def __init__(self, pretrain, width_mult, lstm_layers, lstm_hidden, bidirectional=True, dropout=True):
        ...
        state_dict_mobilenet = torch.load('networks/mobilenet_v2.pth.tar')
        if pretrain:
    ...
    def forward(self, x, lengths=None):
        ...
        self.hidden = self.init_hidden(batch_size)      # LSTM 네트워크에 쓰임, 설정

        # CNN forward
        c_in = x.view(batch_size * timesteps, C, H, W)  # [12, 64, 3, 160, 160] -> [768, 3, 160, 160]
        c_out = self.cnn(c_in)                          # [768, 3, 160, 160] -> [768, 1280, 5, 5]
        c_out = c_out.mean(3).mean(2)                   # [768, 1280, 5, 5] -> [768, 1280, 5] -> [768, 1280]
        if self.dropout:
            c_out = self.drop(c_out)

        # LSTM forward
        r_in = c_out.view(batch_size, timesteps, -1)    # [768, 1280] -> [12, 64, 1280]
